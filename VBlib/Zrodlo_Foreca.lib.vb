...
Public Class Source_Foreca
    ...
    Protected Overrides ReadOnly Property SRC_RESTURI_BASE_PKAR As String = "https://www.foreca.pl/World"
    ...
    Private Sub ExtractFirstItem(sPageFragment As String, sPomiar As String)
        ...
        oNew.sAdres = sPageFragment.SubstringBetweenExclusive("<a href=""/", """")
        ...
        oNew.sCurrValue = sPageFragment.SubstringBetweenExclusive("temp_c", "<").Replace("+", "").Replace("&deg;", "")
    ...
    Private Sub ExtractLastItem(sPageFragment As String, sPomiar As String)

        Dim iInd As Integer = sPageFragment.IndexOf("</ul")
        iInd = sPageFragment.LastIndexOf("<a href", iInd)
        sPageFragment = sPageFragment.Substring(iInd)
        ExtractFirstItem(sPageFragment, sPomiar)

    End Sub


    Public Overrides Async Function GetDataFromFavSensorAsync(ByVal sId As String, ByVal sAddit As String, ByVal bInTimer As Boolean, oPos As pkar.BasicGeopos) As Task(Of Collection(Of JedenPomiar))
        ...
        If iInd > 0 Then
            ExtractFirstItem(sPage.Substring(iInd), "Tmax")
            ExtractLastItem(sPage.Substring(iInd), "Tmin")
        End If
